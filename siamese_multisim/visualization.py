...
def plot_kshot_confusion_matrices(
    kshot_results: Dict[int, Dict],
    output_dir: str,
    filename: str = 'confusion_matrices_kshot.png'
):
    """
    Plot confusion matrices for multiple K-shot values in a grid.
    
    Args:
        kshot_results: Dict mapping K -> {'targets': array, 'predictions': array, 'class_names': list, 'accuracy': float}
        output_dir: Output directory
        filename: Output filename
    """
    k_values = sorted(kshot_results.keys())
    n_k = len(k_values)
    
    # Create grid: 2 rows (counts and normalized) x n_k columns (one per K value)
    fig, axes = plt.subplots(2, n_k, figsize=(6*n_k, 12))
    if n_k == 1:
        axes = axes.reshape(2, 1)
    
    for idx, k in enumerate(k_values):
        result = kshot_results[k]
        ...
        cm = confusion_matrix(y_true, y_pred)
        disp1 = ConfusionMatrixDisplay(cm, display_labels=class_names)
        disp1.plot(ax=axes[0, idx], cmap='Blues', values_format='d', colorbar=False)
        axes[0, idx].set_title(f'{k}-shot (Acc: {acc:.3f})', fontsize=12)
        axes[0, idx].tick_params(axis='x', rotation=45, labelsize=9)
        ...
        cm_norm = confusion_matrix(y_true, y_pred, normalize='true')
        disp2 = ConfusionMatrixDisplay(cm_norm, display_labels=class_names)
        disp2.plot(ax=axes[1, idx], cmap='Blues', values_format='.2f', colorbar=False)
        axes[1, idx].set_title(f'{k}-shot (Normalized)', fontsize=12)
        axes[1, idx].tick_params(axis='x', rotation=45, labelsize=9)
    ...
    logger.info(f"K-shot confusion matrices saved to {output_dir}/{filename}")
...
def generate_all_plots(
    ...
    output_dir: str,
    test_results_9class: Dict = None,
    kshot_results: Dict[int, Dict] = None
):
    """
    ...
    Args:
        ...
        y_test: True test labels (5-class)
        predictions: Predicted labels (5-class)
        class_names: Class names (5 training classes)
        model: Trained model
        x_test: Test features (5-class)
        device: Torch device
        nway_results: N-way K-shot results
        simulation_results: Real-world simulation results
        output_dir: Output directory
        test_results_9class: Optional 9-class test results dict
        kshot_results: Optional dict mapping K -> test results for K-shot classification
    """
    ...
    # Confusion matrix for 5-class (training classes)
    plot_confusion_matrix(y_test, predictions, class_names, output_dir, 
                         filename='confusion_matrix_5class.png')
    
    # Confusion matrix for 9-class (all classes) if available
    if test_results_9class is not None:
        plot_confusion_matrix(
            test_results_9class['targets'],
            test_results_9class['predictions'],
            test_results_9class['class_names'],
            output_dir,
            filename='confusion_matrix_9class.png'
        )
    
    # K-shot confusion matrices if available
    if kshot_results is not None and len(kshot_results) > 0:
        plot_kshot_confusion_matrices(kshot_results, output_dir)
