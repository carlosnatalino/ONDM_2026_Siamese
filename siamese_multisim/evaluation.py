...
def evaluate_nway_kshot(
    ...
    for episode in range(n_episodes):
        ...
        # Classify using cosine similarity (simple and effective for normalized embeddings)
        # Compute cosine similarity between each query and each prototype
        similarities = F.cosine_similarity(
            queries.unsqueeze(1),      # (n_queries, 1, emb_dim)
            prototypes.unsqueeze(0),   # (1, n_way, emb_dim)
            dim=2                       # (n_queries, n_way)
        )
        
        predictions = similarities.argmax(dim=1)  # highest similarity = predicted class
...
class NoveltyDetector:
    ...
    def fit(
        ...
        x: np.ndarray,
        y: np.ndarray,
        statistical_k: float = 2.5,
        calibrate: bool = False,
        fixed_percentile: float = 97.5
    ):
        """
        ...
        # Distance to nearest prototype (using cosine distance = 1 - cosine_similarity)
        all_norm = F.normalize(all_embeddings, p=2, dim=1)
        proto_norm = F.normalize(self.prototypes, p=2, dim=1)
        similarities = torch.mm(all_norm, proto_norm.t())
        distances = 1.0 - similarities  # convert similarity to distance
        min_distances, _ = distances.min(dim=1)
        min_distances_np = min_distances.cpu().numpy()
        
        self.known_distances = min_distances_np
        
        # Compute thresholds
        mean_dist = np.mean(min_distances_np)
        std_dist = np.std(min_distances_np)
        
        # Statistical threshold: mean + k * std
        self.threshold_statistical = mean_dist + statistical_k * std_dist
        
        # Fixed threshold: high percentile of known distances
        self.threshold_fixed = np.percentile(min_distances_np, fixed_percentile)

        # Optional learned threshold via pseudo-novel calibration (2nd-nearest distances)
        if calibrate and distances.shape[1] > 1:
            sorted_distances, _ = torch.sort(distances, dim=1)
            pseudo_novel = sorted_distances[:, 1].cpu().numpy()

            labels = np.concatenate([
                np.zeros_like(min_distances_np, dtype=int),
                np.ones_like(pseudo_novel, dtype=int)
            ])
            candidates = np.percentile(
                np.concatenate([min_distances_np, pseudo_novel]),
                np.linspace(50, 99, 25)
            )
            ...
            for thresh in candidates:
                preds = (np.concatenate([min_distances_np, pseudo_novel]) > thresh).astype(int)
            ...
            self.threshold_learned = best_threshold
            self.calibration_stats = {
                'f1': best_f1,
                'precision': best_prec,
                'recall': best_rec,
                'n_known': len(min_distances_np),
                'n_pseudo_novel': len(pseudo_novel)
            }
            logger.info(
                "  Learned threshold (pseudo-novel calib): "
                f"thr={best_threshold:.4f} | F1={best_f1:.4f} | "
    ...
    def detect(
        ...
        # Compute distances to prototypes (using cosine distance)
        all_norm = F.normalize(all_embeddings, p=2, dim=1)
        proto_norm = F.normalize(self.prototypes, p=2, dim=1)
        similarities = torch.mm(all_norm, proto_norm.t())
        distances = 1.0 - similarities  # convert similarity to distance
        ...
        novel_learned = np.zeros_like(min_distances, dtype=bool)

        if self.threshold_learned is not None:
            novel_learned = min_distances > self.threshold_learned
    ...
    def evaluate_novelty_detection(
        ...
        for method in ['fixed', 'statistical', 'learned']:
            if method == 'learned' and self.threshold_learned is None:
                continue
...
class RealWorldSimulator:
    ...
    def classify_with_pool(
        ...
        similarities = torch.mm(queries_norm, prototypes_norm.t())
